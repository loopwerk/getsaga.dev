  ...
  return layout(title: "API Reference", activePage: .api) {
    div(class: "mx-auto max-w-5xl px-6 pt-24 pb-16 grid grid-cols-1 md:grid-cols-[220px_1fr] gap-12") {
      ...
      main(class: "doc-content min-w-0") {
        ...
        a(class: "inline-flex items-center gap-2 rounded-lg bg-accent px-7 py-3 text-sm font-semibold text-white! transition-all hover:-translate-y-px hover:bg-accent-hover hover:shadow-lg", href: "/docs/releasenotes/\(latestReleaseMajor(allItems: context.allItems)).x/") {
          "Release notes"
        }
      }
    }
...
func apiSidebar(currentSlug: String, allItems: [AnyItem]) -> Node {
  ...
  return aside(class: "doc-sidebar") {
    SymbolKind.allCases.filter(\.isTopLevel).compactMap { kind -> [Node]? in
      ...
      return [
        h4(class: "text-xs font-semibold uppercase tracking-wide text-zinc-500 mt-6 mb-2 first:mt-0 pl-3") { kind.pluralDisplayName },
